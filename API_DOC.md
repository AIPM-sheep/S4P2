...
根据提供的路径标识符（如 `devops-engineer`），获取该学习路径下的所有教学资源节点，并按学习顺序排列。
...
* **请求参数**: 无
* **成功返回示例 (Success 200 OK)**:
    * 当 `roadmap_slug` 为 `devops-engineer` 时，返回的数据结构如下：
...
* **失败返回示例 (Error 404 Not Found)**:
    * 当提供的 `roadmap_slug` 不存在时，例如 `/api/roadmaps/non-existent-slug`：
